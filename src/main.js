...
const services = [
  { id:'service-measurement', nav:'数据分析与测量', number:'01/', title:'数据测量与分析', lead:'建立可信赖的数据分析体系', support:'支持平台', description:'整合网站、App、CRM、会员系统与业务数据，帮助企业全面掌握用户行为、营销表现与业务增长情况。', list:['GA4','神策分析','Google Analytics 360','服务器端追踪','可视化仪表板'], image:'service-measurement-figma.png', imageY:'-17%', imageH:'124.667%' },
  { id:'service-media', nav:'营销与媒体优化', number:'02/', title:'营销与媒体优化', lead:'提升营销投资回报率', support:'支持平台', description:'通过归因分析、营销测量与媒体优化，帮助企业实现更科学的预算分配与营销决策。', list:['营销组合模型（MMM）','Meridian','Display & Video 360','Search Ads 360','受众策略规划','广告投放优化'], image:'service-media-figma.png', imageY:'0%', imageH:'124.667%' },
  { id:'service-ai', nav:'AI与智能决策', number:'03/', title:'AI 与智能决策', lead:'让 AI成为营销增长的新引擎', support:'支持能力', description:'结合人工智能与数据科学能力，帮助企业实现洞察自动化、预测分析与智能決策。', list:['AgentHub','GEO 优化','预测模型','AI洞察分析','自动化报表'], image:'service-ai-figma.png', imageY:'-15%', imageH:'124.667%' },
  { id:'service-cloud', nav:'数据云与客户数据平台', number:'04/', title:'数据云与客户数据平台', lead:'打造统一的数据资产与客户视图', support:'支持平台', description:'打通企业内部与外部数据来源，建立可治理、可扩展的数据基础设施，为 AI 与数字化转型奠定基础。', list:['BigQuery','CDP','神策 CDP','数据仓库','数据治理','第一方数据策略'], image:'service-cloud-figma.png', imageY:'-2.833%', imageH:'125.667%' }
]

const serviceCard = item => `<article class="service-card" id="${item.id}"><div class="service-copy"><small>${item.number}</small><h3>${item.title}</h3><h4 class="service-lead">${item.lead}</h4><p>${item.description}</p><h4 class="service-support">${item.support}</h4><ul>${item.list.map(value=>`<li>${value}</li>`).join('')}</ul></div><div class="service-visual" style="--service-image-y:${item.imageY};--service-image-h:${item.imageH}"><img src="${A}${item.image}" alt="${item.title}"></div></article>`

const caseStudies = [
  { image:'case1.jpg', alt:'奢侈品案例', category:'奢侈品 & 美容', title:'主要奢侈品品牌：利用AI焦点小组捕捉品牌认知', href:'/cases/luxury-ai' },
  { image:'case2.jpg', alt:'雅诗兰黛案例', category:'奢侈品 & 美容', title:'雅诗兰黛：部署一个+8%媒体ROI和-10%营销预算的子午线MMM', href:'/cases/estee-lauder' },
  { image:'case3.png', alt:'雷诺汽车案例', category:'汽车', title:'雷诺集团：利用CAPI和PII增强的转化测量提升活动表现', href:'/cases/auto-1' }
]

const caseMetrics = [['+6.3%','与非优化场景相比，增量收入提升'],['+8%','媒体投资回报率'],['-10%','营销预算']]
...
const casesPageStudies = [
  { image:'cases-luxury-ai.png', alt:'主要奢侈品品牌人工智能焦点小组案例', category:'奢侈品 & 美容', title:'主要奢侈品品牌：利用AI焦点小组捕捉品牌认知', href:'/cases/luxury-ai' },
  { image:'cases-estee-lauder.png', alt:'雅诗兰黛媒体投资回报率案例', category:'奢侈品 & 美容', title:'雅诗兰黛：部署一个+8%媒体ROI和-10%营销预算的子午线MMM', href:'/cases/estee-lauder' },
  { image:'cases-global-beauty.png', alt:'美妆品牌全漏斗营销案例', category:'奢侈品 & 美容', title:'美妆品牌：以全漏斗营销策略开拓海外新市场，实现国际化增长', href:'/cases/global-beauty' },
  { image:'cases-blaire.jpg', alt:'奢侈品牌营销分类体系案例', category:'奢侈品 & 美容', title:'奢侈品牌：通过营销分类体系优化广告活动分析效率', href:'/cases/blaire' },
  { image:'cases-dashboard.png', alt:'美妆品牌电商数据看板案例', category:'奢侈品 & 美容', title:'美妆品牌：整合电商数据看板，打造统一的数据分析平台', href:'/cases/beauty-dashboard' },
...
const casesForIndustry = filter => {
  ...
  return casesPageStudies.map((_, index) => {
    ...
    return {...source, href:`/cases/${filter}-${index+1}`, category:label, alt:`${label}成功案例`, title:`${label}品牌：${titleEnds[index]}`}
  })
}
...
const generatedDetailStudy = generatedDetailMatch
  ...
  : null
const detailCurrentStudy = casesPageStudies.find(item => item.href?.endsWith(`/${detailSlug}`)) || generatedDetailStudy || casesPageStudies[0]
...
const detailOtherStudies = [
  {image:'detail-luxury-hero.png',alt:'奢侈品人工智能焦点小组案例',category:'奢侈品 & 美容',title:'主要奢侈品品牌：利用AI焦点小组捕捉品牌认知',href:'/cases/luxury-ai'},
  {image:'detail-estee.png',alt:'雅诗兰黛媒体投资回报率案例',category:'奢侈品 & 美容',title:'雅诗兰黛：部署一个+8%媒体ROI和-10%营销预算的子午线MMM',href:'/cases/estee-lauder'},
  {image:'detail-auto.png',alt:'雷诺汽车转化测量案例',category:'汽车',title:'雷诺集团：利用CAPI和PII增强的转化测量提升活动表现',href:'/cases/auto-1'}
]
  ...
  <div class="site-shell">
    <header class="hero screen" id="top">
      <nav class="nav wrap" aria-label="主导航">
        ...
        <div class="nav-actions"><button class="lang">EN</button>${button('联系我们')}</div>
      </nav>
    ...
    <main>
      <section class="why section" id="why">
        ...
        <div class="why-grid wrap">
          ${[
            ['why-global.svg','全球数据咨询经验','服务国际品牌与跨市场团队，覆盖中国与全球数据生态。','Global Data Consulting'],
            ['why-ecosystem.svg','出海数据生态能力','谷歌分析及营销专家，深度整合 GA4、DV360、CM360、BigQuery 等平台能力。CDP 产品、UI/UX 分析，打造跨平台数据整合和洞察能力。','Global Data Ecosystem'],
            ['why-ai.svg','AI 驱动营销决策','布局 GEO，优化 AI 搜索曝光，结合 AI 与数据科学，提升媒体效率与营销洞察。','AI-Driven Marketing'],
            ['why-china.svg','本土数据生态能力','具备线上数据、企业微信、小程序、CDP 与本地媒体生态整合经验，帮助品牌建立符合中国市场的数据能力体系。','China Data Ecosystem']
          ].map(x=>`<article class="why-card" tabindex="0"><div class="why-card-inner">${whyFace(x,'front')}${whyFace(x,'back')}</div></article>`).join('')}
        </div>
      </section>
      ...
      <section class="cases section" id="cases">
        ...
        <div class="case-progress wrap" aria-hidden="true"><span></span></div>
      </section>

      <section class="contact section" id="contact">
        <div class="contact-grid wrap"><div><div class="section-title"><small>// CONTACT</small><h2>联系我们</h2></div><p class="lead">准备好建立更智能的数据增长体系了吗？<br>与我们的顾问团队聊聊，探索适合您的数据与 AI 增长策略。</p><div class="contact-cards"><article><span class="contact-icon"><img src="${A}contact-mail.svg" alt=""></span><small>// CHAT TO SALES</small><b>contact-cn@fifty-five.com</b></article><article><span class="contact-icon"><img src="${A}contact-phone.svg" alt=""></span><small>// CALL US</small><b>+86 185 0211 0262</b></article></div></div><form><div class="form-row"><label>姓<input placeholder="您的姓氏"></label><label>名<input placeholder="您的名字"></label></div><label>邮箱<input type="email" placeholder="您的邮箱"></label><label>公司<input placeholder="您的公司"></label><label>留言<textarea placeholder="请输入您想咨询的内容"></textarea></label>${button('提交信息','#contact')}</form></div>
      </section>
    </main>
...
const privacySections = [
  ...
  ['2. 我们收集的信息','<p>我们可能会收集以下类型的信息：</p><ul><li>个人信息： 当您通过电子邮件或联系表单与我们联系时，您自愿提供给我们的信息，例如您的姓名、电子邮箱地址和电话号码。</li><li>非个人信息： 当您访问我们的网站时，我们会自动收集某些非个人数据，包括您的 IP 地址、浏览器类型、操作系统和浏览行为。这有助于我们提升网站的性能。</li></ul>'],
  ['3. 我们如何使用您的信息','<p>我们收集的信息将用于以下用途：</p><ul><li>提供并维护我们的网站服务。</li><li>回应您的咨询、意见或请求。</li><li>分析网站流量并优化用户体验。</li></ul>'],
  ['4. Cookie 技术','<p>我们的网站使用 Cookie 来提升您的浏览体验。Cookie 是存储在您设备上的小数据文件。您可以选择通过浏览器设置禁用 Cookie，但这可能会影响网站的部分功能。</p>'],
  ['5. 数据共享与披露','<p>我们不会将您的个人身份信息出售、交易或租借给他人。我们可能会与业务合作伙伴和广告商共享不与任何个人身份信息挂钩的通用汇总人口统计信息。在法律要求的情况下，我们亦可能披露相关信息。</p>'],
  ['6. 数据安全','<p>我们采取适当的安全措施，以防止您的个人信息遭到未经授权的访问、篡改、披露或销毁。但请注意，任何通过互联网传输的方法都无法保证 100% 的绝对安全。</p>'],
  ['7. 本政策的变更','<p>我们可能会不时更新本隐私政策。任何变更都将在此页面上公布，并更新修订日期。</p>'],
  ['8. 联系我们','<p>如果您对本隐私政策有任何疑问，请通过以下方式与我们联系：</p><ul><li>五十五</li><li>电子邮箱：contact-cn@fifty-five.com</li></ul>']
]
...
document.querySelector('#app').innerHTML = isPrivacyPage ? privacyPage : isCaseDetailPage ? caseDetailPage : isCasesPage ? casesPage : homePage
...
document.querySelectorAll('.section-title small').forEach(label => {
  ...
  label.innerHTML = chars.map((char, index) => {
...
requestAnimationFrame(() => requestAnimationFrame(() => heroEntrances.forEach(element => element.classList.add('is-visible'))))

const revealGroups = [
  '.section-title', '.why-card', '.brand-grid > *', '.client-grid > div',
  '.service-tabs', '.partner-marquee', '.case-track article',
  '.case-progress', '.contact-grid > div', '.contact-grid > form', '.footer-grid > div',
  '.cases-page-intro > p', '.cases-feature', '.cases-filter', '.cases-list-card',
  '.case-detail-summary > *', '.case-detail-anchor', '.case-detail-section', '.case-detail-stats article'
]
...
if (!reduced && innerWidth > 768) {
  let current = scrollY, target = scrollY, raf = 0
  const tick = () => {
  ...
  addEventListener('wheel', e => {
    if (e.ctrlKey) return
    e.preventDefault()
    target = Math.max(0, Math.min(document.documentElement.scrollHeight - innerHeight, target + e.deltaY * .78))
...
if (caseTrack && caseProgress && caseProgressThumb) {
  ...
  const runCaseInertia = () => {
    caseVelocity *= .9
    if (Math.abs(caseVelocity) < .18) { caseInertiaFrame = 0; return }
    const previous = caseOffset
    setCaseOffset(caseOffset + caseVelocity)
    if (caseOffset === previous) { caseInertiaFrame = 0; return }
  ...
  const finishCaseDrag = event => {
    ...
    if (wasDragging && !reduced && Math.abs(caseVelocity) >= .18) caseInertiaFrame = requestAnimationFrame(runCaseInertia)
  }
  addEventListener('pointerup', finishCaseDrag)
  addEventListener('pointercancel', finishCaseDrag)
  caseTrack.addEventListener('click', event => {
...
const contactDrawerTriggers = [...document.querySelectorAll('.nav-actions .button,.footer-contact-trigger')]
...
contactSelectTrigger?.addEventListener('click', () => contactSelect?.classList.contains('is-open') ? closeContactSelect() : openContactSelect())
contactSelectOptions.forEach(option => option.addEventListener('click', () => {
...
contactSelect?.addEventListener('keydown', event => {
  ...
  if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); closeContactSelect(true); return }
  if (!contactSelect?.classList.contains('is-open') && ['ArrowDown','Enter',' '].includes(event.key)) { event.preventDefault(); openContactSelect(); return }
  if (activeIndex < 0) return
  if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
    event.preventDefault()
    const direction = event.key === 'ArrowDown' ? 1 : -1
    contactSelectOptions[(activeIndex + direction + contactSelectOptions.length) % contactSelectOptions.length]?.focus()
  }
  if (event.key === 'Home' || event.key === 'End') { event.preventDefault(); contactSelectOptions[event.key === 'Home' ? 0 : contactSelectOptions.length - 1]?.focus() }
})
...
contactDrawerTriggers.forEach(trigger => {
  ...
  trigger.addEventListener('click', event => { event.preventDefault(); openContactDrawer() })
})
...
contactDrawer?.addEventListener('keydown', event => {
  ...
  const focusable = [...contactDrawer.querySelectorAll('button:not([disabled]):not([tabindex="-1"]),input:not([disabled]),textarea:not([disabled])')]
...
addEventListener('keydown', event => { if (event.key === 'Escape') closeContactDrawer() })

const cookieNotice = document.querySelector('.cookie-notice')
const cookieConsentKey = 'fifty-five-cookie-consent'
let savedCookieChoice = ''
try { savedCookieChoice = localStorage.getItem(cookieConsentKey) || '' } catch {}
...
articleModalTriggers.forEach(trigger => {
  ...
  trigger.addEventListener('click', event => { event.preventDefault(); openArticleModal() })
})
articleModalClose?.addEventListener('click', closeArticleModal)
articleModalBackdrop?.addEventListener('click', closeArticleModal)
articleModal?.addEventListener('keydown', event => {
  if (event.key === 'Escape') { event.stopPropagation(); closeArticleModal(); return }
  if (event.key !== 'Tab') return
...
addEventListener('keydown', event => { if (event.key === 'Escape') closeArticleModal() })
...
const setDetailAnchor = id => detailAnchorLinks.forEach(link => {
...
if (detailSections.length && 'IntersectionObserver' in window) {
  const detailAnchorObserver = new IntersectionObserver(entries => {
